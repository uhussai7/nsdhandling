...
def load_beta_file(filename, voxel_mask=None, zscore=True, run_zscore=False, hemi=None):
    from .file_utility import load_mask_from_nii
    if ".mat" in filename:
        ...
        print(filename)
        if 'flat' in filename:
            beta = np.load(filename)/300
            beta = np.reshape(beta,[beta.shape[0],-1])
    elif ".mgh" in filename:
        #if hemi==None:
        #    raise ValueError('Looks like you want an overlay file. Please provide a hemisphere')
        #this is for overlays so have to account for left right hemisphere
        #if hemi in filename:
        values = load_mask_from_nii(filename)
        beta = values.reshape([values.shape[-1],values.shape[0]]).astype(np.float32) / 300.
    else:
...
def load_betas(folder_name, zscore=False, voxel_mask=None, up_to=0, load_ext='.mat',hemi=None):
    '''load beta value in the structure of the NSD experiemnt'''
    from .file_utility import list_files
    matfiles, betas = [], []
    k = 0
    for filename in list_files(folder_name):
        ...
        if 'betas' in filename_no_path and load_ext in filename_no_path:
            if hemi == None:
                ...
                print(filename)
                matfiles += [filename, ]
                betas += [load_beta_file(filename, voxel_mask=voxel_mask, zscore=zscore,hemi=hemi), ]
            elif hemi in filename_no_path:
                k += 1
                if up_to > 0 and k > up_to:
                    break
                print(filename)
